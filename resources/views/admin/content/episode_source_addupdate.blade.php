    ...
    <div class="row">
        <div class="col-12 col-md-12 col-lg-12">
          ...
          <div class="card  card-stats">
            ...
            <div class="card-body">
            <h4 class="text-center mb-0"> {{$data['episode_title']}} Source</h4>
            </div>
          </div>
          @endif
          <div class="card">
        ...
        <div class="col-12">
            ...
            <div class="card-body">
                <div class="pull-right">
                    <div class="buttons"> 
                    <button class="btn btn-primary text-light" data-toggle="modal" data-target="#SourceModal" data-whatever="@mdo" >Add Source</button>
                    </div>
                </div>
...
<div class="modal fade" id="SourceModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form id="addUpdateEpisodeSource" method="post" enctype="multipart">
        {{ csrf_field() }}
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-success source_add" >Save</button>
...
$(document).ready(function (){
  var dataTable = $('#source-content-listing').dataTable({
    ...
    'ajax': {
        'url':'{{ route("showEpisodeSourceList") }}',
        'data': function(data){
  ...
  $(document).on('change', '#source_type', function() {
    ...
    var videoPreview = function(input) {
        ...
        for (i = 0; i < filesAmount; i++) {
            ...
            var formdata = new FormData($("#addUpdateEpisodeSource")[0]);
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success == 1) {
                        ...
                        $(input).parent().parent().find('.source_preview').html('<div class=""><video width="150" height="150" class="displayimg1" controls=""> <source src="'+data.default_path+'/'+data.source_video+'" type="video/mp4"> </video> </div>');
                        $(".source_video").val("");
                    } 
                }
            });
    ...
    $('#SourceModal').on('hidden.bs.modal', function(e) {
        $("#addUpdateEpisodeSource")[0].reset();
        $('.modal-title').text('Add Source');
        $('#source_id').val("");
        $('.source_preview').html("");
        var validator = $("#addUpdateEpisodeSource").validate();
        validator.resetForm();
    });

    $("#source-content-listing").on("click", ".updateEpisodeSource", function() {
    ...
    $("#addUpdateEpisodeSource").validate({
      rules: {
        source_title: {
          required: true,
        },
  ...
  $(document).on('submit', '#addUpdateEpisodeSource', function (e) {
      e.preventDefault();
      if (user_type == 1) {
        var formdata = new FormData($("#addUpdateEpisodeSource")[0]);
        $('.loader').show();
        $.ajax({
            url: '{{ route("addUpdateEpisodeSource") }}',
            type: 'POST',
            data: formdata,
            dataType: "json",
            contentType: false,
    ...
    $(document).on('click', '.DeleteEpisodeSource', function (e) {
        e.preventDefault();
        ...
        function(isConfirm){
            if (isConfirm){
              if (user_type == 1) {
                $('.loader').show();
                $.ajax({
                    url: '{{ route("deleteEpisodeSource") }}',
                    type: 'POST',
                    data: {"source_id":source_id,"episode_id":episode_id},
                    dataType: "json",
                    cache: false,
                    success: function (data) {
                        $('.loader').hide();
